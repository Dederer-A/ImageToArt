# Plan: Add Native Image Sharing Support (up to 10 images) via Capacitor Share Target

## Objective
Enable users to share up to 10 images from the native iOS "Photos" app (or other apps) directly into ImageToArt via the native share sheet (`CapacitorShareTarget`). Save all shared images through persistence, but open only the last received image in the editor. Refactor document initialization to avoid code duplication between standard file import and shared image import.

---

## Proposed Changes

### 1. Workplace Store Refactoring (`src/workplace/index.ts`)
...
### 2. Share Target Listener in Application Entry Point (`src/App.vue`)
...
## Verification Plan
1. Run `npm run build` (`vue-tsc -b && vite build`) to verify TypeScript type checking and Vite build pass without errors.
